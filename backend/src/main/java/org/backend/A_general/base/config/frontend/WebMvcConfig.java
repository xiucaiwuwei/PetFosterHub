...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(@NonNull ResourceHandlerRegistry registry) {
        try {
            ...
            String location = "file:" + new File(normalizedPath).getAbsolutePath() + "/";
            ...
            throw new RuntimeException("无法配置静态资源处理器", e);
        }
    }
    ...
    private void validateUploadPath(String path) throws IllegalArgumentException {
        ...
        try {
            ...
            // 检查规范化后的路径是否还在原始路径内，防止路径遍历
            String absolutePath = Paths.get(path).toAbsolutePath().normalize().toString();
            if (!absolutePath.endsWith(normalizedPath)) {
                throw new IllegalArgumentException("上传路径包含非法字符");
            }
        } catch (Exception e) {
            throw new IllegalArgumentException("上传路径格式不正确: " + e.getMessage());
        }
    }
}