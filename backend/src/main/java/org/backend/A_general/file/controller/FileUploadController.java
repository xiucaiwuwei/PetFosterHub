...
public class FileUploadController extends BaseController {
    ...
    public ResponseEntity<BaseResponse<String>> uploadAudio(@RequestParam("file") MultipartFile file, Authentication authentication) {
        try {
            // 参数验证
            if (file == null || file.isEmpty()) {
                return super.failure("上传文件不能为空");
            ...
            FileUpload uploadedFile = fileUploadService.uploadFile(file, "MESSAGE", null, false, user);
            // 返回文件URL（实际应用中应该返回完整的访问URL）
            logger.info("用户 {} 上传音频成功，文件ID: {}", userId, uploadedFile.getId());
            return super.success("音频上传成功", uploadedFile.getFilePath());
        } catch (Exception e) {
            logger.error("音频上传失败: {}", e.getMessage());
            return super.failure("音频上传失败: " + e.getMessage());
        }
    }

    @PostMapping("/api/upload/document")
    @Operation(summary = "上传文档文件")
    public ResponseEntity<BaseResponse<String>> uploadDocument(@RequestParam("file") MultipartFile file, Authentication authentication) {
        try {
            // 参数验证
            if (file == null || file.isEmpty()) {
                return super.failure("上传文件不能为空");
            ...
            Long userId = getCurrentUserId(authentication);
            User user = createUser(userId);
            FileUpload uploadedFile = fileUploadService.uploadFile(file, "DOCUMENT", null, false, user);
            // 返回文件URL（实际应用中应该返回完整的访问URL）
            logger.info("用户 {} 上传文档成功，文件ID: {}", userId, uploadedFile.getId());
            return super.success("文档上传成功", uploadedFile.getFilePath());
        } catch (Exception e) {
            logger.error("文档上传失败: {}", e.getMessage());
            return super.failure("文档上传失败: " + e.getMessage());
        }
    }

    @PostMapping("/api/upload/file")
    @Operation(summary = "通用文件上传接口")
    public ResponseEntity<BaseResponse<String>> uploadAnyFile(
        ...
        try {
            ...
            Long userId = getCurrentUserId(authentication);
            User user = createUser(userId);
            FileUpload uploadedFile = fileUploadService.uploadFile(file, "GENERAL", null, false, user);
            // 返回文件URL（实际应用中应该返回完整的访问URL）
            logger.info("用户 {} 上传文件成功，文件ID: {}", userId, uploadedFile.getId());
            return super.success("文件上传成功", uploadedFile.getFilePath());
        } catch (Exception e) {
            logger.error("文件上传失败: {}", e.getMessage());
            return super.failure("文件上传失败: " + e.getMessage());
        }
    ...
    public ResponseEntity<BaseResponse<FileUpload>> getFileInfo(@PathVariable Long fileId, Authentication authentication) {
        try {
            ...
            // 使用FileSecurityUtils检查访问权限
            if (!FileSecurityUtils.hasAccessPermission(file, userId)) {
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable Long fileId, Authentication authentication) {
        try {
            ...
            // 使用FileSecurityUtils检查访问权限
            if (!FileSecurityUtils.hasAccessPermission(fileUpload, userId)) {
                logger.warn("用户 {} 无权限下载文件 {}", userId, fileId);
                return ResponseEntity.notFound().build();
            }

            try {
                // 使用FileUtils检查文件是否存在并获取资源
                String filePath = fileUpload.getFilePath();
                if (!FileUtils.fileExists(filePath)) {
                    logger.warn("文件资源不存在，文件路径: {}", filePath);
                    return ResponseEntity.notFound().build();
                }

                Resource resource = FileUtils.getFileResource(filePath);

                logger.info("用户 {} 下载文件成功，文件ID: {}", userId, fileId);
                return ResponseEntity.ok().contentType(MediaType.parseMediaType(fileUpload.getContentType()))
